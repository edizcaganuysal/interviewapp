...
export default function TranscriptEditPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (res.ok && json?.item) {
        const initial = json.item.parsed_json?.transcript_text ?? "";
        setText(initial);
        setHistory([initial]);
        setTranscriptFilename(json.item.parsed_json?.transcript_filename ?? null);
      }
    })();
  }, []);
  ...
  async function runAi() {
    if (!text.trim()) {
      setError("Transcript text is empty. Paste your transcript first.");
      return;
    }
    setLoading(true);
    setError(null);
  ...
  async function save() {
    ...
    setSaving(true);
    setError(null);
    const filename = transcriptFilename ? `${transcriptFilename.replace(/\.[^/.]+$/, "")}-edited.pdf` : "transcript-edited.pdf";
    ...
    const res = await fetch("/api/cv/save", {
      ...
      body: JSON.stringify({
        ...
        transcript_file_base64: base64,
        transcript_file_mime: "application/pdf",
      }),
    });
    const json = await res.json().catch(() => null);
    if (!res.ok) {
  ...
  return (
    <div className="page" style={{ display: "grid", gap: 12 }}>
      <div className="section-title">
        <div>
          <h1 style={{ margin: 0 }}>Transcript Editor</h1>
          <div style={{ color: "#475569" }}>Paste transcript text; AI will clean formatting and clarity.</div>
        </div>
      ...
      <div className="card" style={{ display: "grid", gap: 10 }}>
        ...
        <textarea
          ...
          rows={18}
          placeholder="Paste and edit transcript text here..."
        />
      </div>
