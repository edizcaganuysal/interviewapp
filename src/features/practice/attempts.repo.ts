...
export async function upsertSkillState(
  ...
  const { data: current } = await supabase
    ...
    .select("estimated_level,confidence,evidence_count")
    .eq("user_id", userId)
    .eq("skill_id", skillId)
    .single();

  const estimated_level = Math.min(10, Math.max(0, (current?.estimated_level ?? 0) + delta));
  const confidence = Math.min(1, Math.max(0, (current?.confidence ?? 0.4) + delta / 10));
  const evidence_count = (current?.evidence_count ?? 0) + 1;
